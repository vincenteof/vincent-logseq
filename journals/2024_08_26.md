- [[tornado cash]] 代码分析 #[[Crypto Roadmap]]
	...
	- cli 相关代码
		...
		- `withdraw` 提现时会根据 note 反解出 nullifier 和 secret，然后生成对应的 snark proof，调用合约  的 `withdraw` 时候传入对应的 proof 给合约验证。
			- `generateSnarkProof` 实际上会调用 withdraw 的证明电路去生成 proof，它做的主要是生成 input.json, 因为 nullifier 和 secret 已经由 note 解出，因此他需要提供根 merkle tree 有关的参数。为了提供 merkle tree 的相关参数，需要
		-