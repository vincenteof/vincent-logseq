...
-
- 还有一些 case，比如选举和拍卖，这种情况我只想暴露出一个最终的结果而不想透露出其他信息。抽象一下这个行为，在整个行为中会有 n 个参与者，x1 ... xn，目标是要计算一个结果 f(x1, x2, ... , xn)，如何不泄露任何一个输入？
...
- 一些被广泛破解的例子：
	...
	- vigener cipher，将 k 扩展到和密文 m 一样长。然后把每一位的 k 和 m 相加得到结果，解密就是相减
...
- Stream Ciphers：making OTP practical
	...
	- epsilon 在学术上有更严格的关于 negligible 和 non-negligible 的定义：
		...
		- non-neg：
			- 存在某个 d 使得 epsilon(x) >= 1 / x^d
		- neg：
			- 对任意 d，x >= 某个范围之后 epsilon(x) <= 1 / x^d
-
-
- 对 stream cipher 的一些攻击：
	- two time pad 一个 k 用两次：
		- 因为 C1 = m1 xor PRG(k)
			- C2 = m2 xor PRG(k)
		- 攻击拿到这两个密文以后直接做一次 xor，直接能拿到 m1 xor m2 的结果
		- 因为英语和 ASCII 设计的冗余，有办法可以直接拿到 m1 与 m2 的结果
		- 针对这种攻击，永远不用把一个 stream cipher key 多次使用
	- no integrity （OTP is malleable）
		- 假设针对 m 做了次加密 m xor k, 攻击者截断了这条信息，更改为 (m xor k) xor p
		- 接收方接收到这消息以后，解密出来的结果是 m xor p，他得到的是被篡改的信息且无法被发现
		- 比如将 bob 篡改为 eve，只需要 xor 07 19 07
-
-
- stream cipher 的实际案例：
	- RC4（软件）可被破解
	- CSS（硬件）可别破解
	- eStream（2008，相对现代的算法，是比较安全的）:
		- PRG：{0, 1}^s * R -> {0, 1}^ n
		- n >> s
		- 基于 seed + nonce
		- R 是 nonce：a non-repeating value for a give key
		- E(k, m; r) = m xor PRG(k; r)
		- (k, r) 的组合不会重复使用，k 可以重复使用
	- eStream: Salsa 20：
		- PRG：{0, 1}^128 or 256 * {0, 1}^64 -> {0, 1}^n
		- seed * nonce
		- max n = 2^73 bits
		- Salsa20(k; r) := H(k, (r, 0)) || H(k, (r, 1)) || ...
- 基本上所有 stream cipher 的核心都是如何生成 PRG
-
-
- PRG Security Definitions
	- 假设 G: K -> {0, 1}^n 是一个 PRG
	- 需要一个定义去表达：
		- 随机在 K 中选取一个 k 产生 G(k)
		- 随机在 {0, 1}^n 选取一个 r
		- 这两者是 indistinguishable 的
	- statistical test：
		- 它是一个算法 A
		- A(x) 输出 “0” 或 “1”
		- 一些例子：
			- A(x) = 1, iff |numOfOnes(x) - numOfZeros(x)| <= 10 * sqrt(n)
			- A(x) = 1, iff |numOfZeros(x) - n/4| <= 10 * sqrt(n)
			- ...
	- advantage:
		- K -> {0, 1}^n 是一个 PRG
		- A 是  {0, 1}^n 上的 stat test
		- adv 是指 Pr[A(G(k)) = 1] - Pr[A(r) = 1]
		- k 是在 K 上随机选取的，r 在 {0, 1}^n 上随机选取的
		- adv 范围在 [0, 1]
		- adv 接近 1 说明 A 把 G 和 random 区分开了
		- 反之接近 0 说明区分不了
		- 例子：
			- A(x) = 0 => Adv[A, G] = 0
			- 假设
				- G 满足对 2/3 的 k，msb(G(k)) = 1
				- A(x) => msb(x) = 1 ? 1 : 0
				- 则 adv = 2/3 - 0.5 = 1/6
	- Secure PRG：
		- G: K -> {0, 1}^n
		- 这个 PRG 是 secure 的如果
		- 任意 efficient 的 stat test A 都满足 Adv[A, G] is neg
	- 是否存在 provable 的 secure PRG：
		- unknown
		- 等价于 P != NP
	- secure PRG is unpredictable:
		- PRG predictable => PRG is insecure
		- 假设 A 是一个高效预测 PRG 的算法满足：
			- testR = A(G(k))
			- Pr[slice(testR, i) = slice(G(k), i +1, i + 2)] = 1/2 + epsilon
			- epsilon is non-neg (e.g. epsilon = 1/1000)
		- 定义一个 stat test B:
			- 如果 A(slice(x, i)) = slice(x, i + 1, i + 2) 输出 1
			- 否则输出 0
			- 就是查看 A 的预测结果是否准确
		- 所以
			- 针对 {0,1}^n 随机选取的 r，Pr[B(r) = 1] = 1/2
				- 因为是随机串，所以 i+1 位与前 i 位是完全独立的
			- K 里随机选取 k，Pr[B(G(k)) = 1] > 1/2 + epsilon
			- Adv[B, G] > epsilon，which is non-neg
	- Yao: an unpredictable PRG is secure
		- G: K -> {0, 1}^n
		- 对任意位置 i，如果 PRG 在 i 是不可预测的，那么 G 是 secure 的。
		- 不 secure 的一定 predictable
		- https://crypto.stackexchange.com/questions/18043/an-unpredictable-prg-is-secure-theorem-yao82
-
-
- Semantic Security
	- 假设 attacker 当前的能力是：获取整个密文
	- 一些关于关于加密算法是否安全的定义并不优雅，比如：
		- attacker 不能获得 secret key
		- attacker 不能恢复整个 plaintext
	- shannon 定义了 perfect secrecy，但是太过严格，需要 k 与 m 一样长，不适合 stream cipher，我们需要放宽定义
		- 这里引入 semantic security，它大意是说两个 msg，m1 与 m2，它们加密的结果 c1 与 c2，无法通过 c1 与 c2 猜测出原始的 msg 对应的是哪个
		- 形式化定义如下：
		- 定义两个实验 EXP(0) 与 EXP(1)
		- m0 与 m1 属于 M 且长度相同
		- k <- K，b 为 0 或 1，c <- E(k, mb)
		- Wb = [event that EXP(b) = 1]
		- Adv = abs(Pr[W0] - Pr[W1]) 这个值在 [0, 1]
		- 靠近 0 说明无法区分
		- E is semantically secure if for all efficient A
			- Adv[A, E] is negligible
	- OTP is semantically secure （Sure！）
-
-
- Stream Ciphers are semantically secure
	- G: K -> {0, 1} ^n is a secure PRG =>
